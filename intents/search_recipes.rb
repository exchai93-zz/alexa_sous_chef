...
intent "SearchRecipes" do
    ...
    if unavailable_ingredients.empty?
      ...
      response_text = "Here are the recipes " + formatted_recipes.each_with_index.map { |recipe, i| "Recipe number #{i + 1}, #{recipe.keys}" }.flatten.join(', ') + "Which recipe number would you like to cook?"
